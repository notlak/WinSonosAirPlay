...
// ### maybe swap these functions for the ones in base64.hh???
...
////////// AirPlayRTSPServer implementation //////////

AirPlayRTSPServer*
AirPlayRTSPServer::createNew(UsageEnvironment& env, Port ourPort,
...
Boolean AirPlayRTSPServer::lookupByName(UsageEnvironment& env,
				 char const* name,
				 AirPlayRTSPServer*& resultServer) {
  ...
  resultServer = (AirPlayRTSPServer*)medium;
  return True;
}

void airPlayRtspRegisterResponseHandler(RTSPClient* rtspClient, int resultCode, char* resultString); // forward

// A class that represents the state of a "REGISTER" request in progress:
class RegisterRequestRecord: public RTSPRegisterSender {
public:
  RegisterRequestRecord(AirPlayRTSPServer& ourServer, unsigned requestId,
			char const* remoteClientNameOrAddress, portNumBits remoteClientPortNum, char const* rtspURLToRegister,
			AirPlayRTSPServer::responseHandlerForREGISTER* responseHandler, Authenticator* authenticator,
    ...
    : RTSPRegisterSender(ourServer.envir(), remoteClientNameOrAddress, remoteClientPortNum, rtspURLToRegister,
			 airPlayRtspRegisterResponseHandler, authenticator,
...
private:
  AirPlayRTSPServer& fOurServer;
  unsigned fRequestId;
  AirPlayRTSPServer::responseHandlerForREGISTER* fResponseHandler;
};

void airPlayRtspRegisterResponseHandler(RTSPClient* rtspClient, int resultCode, char* resultString) {
...
unsigned AirPlayRTSPServer::registerStream(ServerMediaSession* serverMediaSession,
...
char* AirPlayRTSPServer::rtspURLPrefix(int clientSocket) const {
...
UserAuthenticationDatabase* AirPlayRTSPServer::setAuthenticationDatabase(UserAuthenticationDatabase* newDB) {
...
Boolean AirPlayRTSPServer::setUpTunnelingOverHTTP(Port httpPort) {
...
portNumBits AirPlayRTSPServer::httpServerPortNum() const {
  return ntohs(fHTTPServerPort.num());
}

char const* AirPlayRTSPServer::allowedCommandNames() {
  //return "OPTIONS, DESCRIBE, SETUP, TEARDOWN, PLAY, PAUSE, GET_PARAMETER, SET_PARAMETER";
	return "ANNOUNCE, SETUP, RECORD, PAUSE, FLUSH, TEARDOWN, OPTIONS, GET_PARAMETER, SET_PARAMETER, POST, GET";
}

Boolean AirPlayRTSPServer::weImplementREGISTER(char const* /*proxyURLSuffix*/, char*& responseStr) {
  // By default, servers do not implement our custom "REGISTER" command:
  responseStr = NULL;
  return False;
}

void AirPlayRTSPServer::implementCmd_REGISTER(char const* /*url*/, char const* /*urlSuffix*/, int /*socketToRemoteServer*/,
				       Boolean /*deliverViaTCP*/, char const* /*proxyURLSuffix*/) {
  // By default, this function is a 'noop'
}

UserAuthenticationDatabase* AirPlayRTSPServer::getAuthenticationDatabaseForCommand(char const* /*cmdName*/) {
  // default implementation
  return fAuthDB;
}

Boolean AirPlayRTSPServer::specialClientAccessCheck(int /*clientSocket*/, struct sockaddr_in& /*clientAddr*/, char const* /*urlSuffix*/) {
  // default implementation
  return True;
}

Boolean AirPlayRTSPServer::specialClientUserAccessCheck(int /*clientSocket*/, struct sockaddr_in& /*clientAddr*/,
...
AirPlayRTSPServer::AirPlayRTSPServer(UsageEnvironment& env,
  ...
  : GenericMediaServer(env, ourSocket, ourPort, reclamationSeconds),
    ...
    fAuthDB(authDatabase), fAllowStreamingRTPOverTCP(True), _airPortExpressKey(nullptr) 
{
	LoadAirPortExpressKey();
}
...
AirPlayRTSPServer::~AirPlayRTSPServer() {
...
Boolean AirPlayRTSPServer::isRTSPServer() const {
  return True;
}

void AirPlayRTSPServer::incomingConnectionHandlerHTTP(void* instance, int /*mask*/) {
  AirPlayRTSPServer* server = (AirPlayRTSPServer*)instance;
  server->incomingConnectionHandlerHTTP();
}
void AirPlayRTSPServer::incomingConnectionHandlerHTTP() {
  incomingConnectionHandlerOnSocket(fHTTPServerSocket);
}

void AirPlayRTSPServer
::noteTCPStreamingOnSocket(int socketNum, AirPlayRTSPClientSession* clientSession, unsigned trackNum) {
...
void AirPlayRTSPServer
::unnoteTCPStreamingOnSocket(int socketNum, AirPlayRTSPClientSession* clientSession, unsigned trackNum) {
...
void AirPlayRTSPServer::stopTCPStreamingOnSocket(int socketNum) {
  // Close any stream that is streaming over "socketNum" (using RTP/RTCP-over-TCP streaming):
  streamingOverTCPRecord* sotcp
    = (streamingOverTCPRecord*)fTCPStreamingDatabase->Lookup((char const*)socketNum);
  if (sotcp != NULL) {
    do {
      AirPlayRTSPClientSession* clientSession
	= (AirPlayRTSPServer::AirPlayRTSPClientSession*)lookupClientSession(sotcp->fSessionId);
...
////////// AirPlayRTSPServer::AirPlayRTSPClientConnection implementation //////////

AirPlayRTSPServer::AirPlayRTSPClientConnection
::AirPlayRTSPClientConnection(AirPlayRTSPServer& ourServer, int clientSocket, struct sockaddr_in clientAddr)
  : GenericMediaServer::ClientConnection(ourServer, clientSocket, clientAddr),
    ...
    fIsActive(True), fRecursionCount(0), fOurSessionCookie(NULL), _pAudioThread(false), _stopAudioThread(false)
{
  resetRequestBuffer();
}

AirPlayRTSPServer::AirPlayRTSPClientConnection::~AirPlayRTSPClientConnection() {
...
AirPlayRTSPServer::AirPlayRTSPClientConnection::ParamsForREGISTER
::ParamsForREGISTER(AirPlayRTSPServer::AirPlayRTSPClientConnection* ourConnection, char const* url, char const* urlSuffix,
...
AirPlayRTSPServer::AirPlayRTSPClientConnection::ParamsForREGISTER::~ParamsForREGISTER() {
  delete[] fURL; delete[] fURLSuffix; delete[] fProxyURLSuffix;
}

bool AirPlayRTSPServer::LoadAirPortExpressKey()
{
...
void AirPlayRTSPServer::AirPlayRTSPClientConnection::handleCmd_OPTIONS()
{
	/*
  ...
  */
	...
	if (challengePos != std::string::npos)
	{
		...
		*/
		...
		int encLen = RSA_private_encrypt(32, rawAppleResp, encAppleResp, fOurRTSPServer._airPortExpressKey, RSA_PKCS1_PADDING);
...
void AirPlayRTSPServer::AirPlayRTSPClientConnection::handleCmd_ANNOUNCE(char const* fullRequestStr)
{
	...
	std::string req(fullRequestStr);

	size_t pos = req.find("rsaaeskey:");
	size_t len = 0;
	...
	if (pos != std::string::npos)
	{
		...
		std::string b64Key = req.substr(pos, len);
		...
		RSA_private_decrypt(encKeyLen, encKey, _aesKey, fOurRTSPServer._airPortExpressKey, RSA_PKCS1_OAEP_PADDING);
...
void AirPlayRTSPServer::AirPlayRTSPClientConnection
::handleCmd_GET_PARAMETER(char const* /*fullRequestStr*/) {
  ...
  // and "RTSPServer::AirPlayRTSPClientConnection", and then reimplement this virtual function in your subclass.)
...
::handleCmd_SET_PARAMETER(char const* /*fullRequestStr*/) {
  ...
  // (If you want to handle this type of "SET_PARAMETER" differently, you can do so by defining a subclass of "AirPlayRTSPServer"
  // and "AirPlayRTSPServer::AirPlayRTSPClientConnection", and then reimplement this virtual function in your subclass.)
	snprintf((char*)fResponseBuffer, sizeof fResponseBuffer,
		"RTSP/1.0 200 OK\r\nServer: AirTunes/105.1\r\nCSeq: %s\r\n\r\n",
		//fCurrentCSeq, dateHeader(), fOurRTSPServer.allowedCommandNames());
		fCurrentCSeq);
...
void AirPlayRTSPServer
::AirPlayRTSPClientConnection::handleCmd_REGISTER(char const* url, char const* urlSuffix, char const* fullRequestStr,
					   Boolean reuseConnection, Boolean deliverViaTCP, char const* proxyURLSuffix) {
  char* responseStr;
...
void AirPlayRTSPServer::AirPlayRTSPClientConnection::handleCmd_bad() {
...
void AirPlayRTSPServer::AirPlayRTSPClientConnection::handleCmd_notSupported() {
  snprintf((char*)fResponseBuffer, sizeof fResponseBuffer,
	   "RTSP/1.0 405 Method Not Allowed\r\nCSeq: %s\r\n%sAllow: %s\r\n\r\n",
	   fCurrentCSeq, dateHeader(), fOurRTSPServer.allowedCommandNames());
}

void AirPlayRTSPServer::AirPlayRTSPClientConnection::handleCmd_notFound() {
  setRTSPResponse("404 Stream Not Found");
}

void AirPlayRTSPServer::AirPlayRTSPClientConnection::handleCmd_sessionNotFound() {
  setRTSPResponse("454 Session Not Found");
}

void AirPlayRTSPServer::AirPlayRTSPClientConnection::handleCmd_unsupportedTransport() {
  setRTSPResponse("461 Unsupported Transport");
}

Boolean AirPlayRTSPServer::AirPlayRTSPClientConnection::parseHTTPRequestString(char* resultCmdName, unsigned resultCmdNameMaxSize,
...
void AirPlayRTSPServer::AirPlayRTSPClientConnection::handleHTTPCmd_notSupported() {
  snprintf((char*)fResponseBuffer, sizeof fResponseBuffer,
	   "HTTP/1.1 405 Method Not Allowed\r\n%s\r\n\r\n",
	   dateHeader());
}

void AirPlayRTSPServer::AirPlayRTSPClientConnection::handleHTTPCmd_notFound() {
  snprintf((char*)fResponseBuffer, sizeof fResponseBuffer,
	   "HTTP/1.1 404 Not Found\r\n%s\r\n\r\n",
	   dateHeader());
}

void AirPlayRTSPServer::AirPlayRTSPClientConnection::handleHTTPCmd_OPTIONS() {
#ifdef DEBUG
...
void AirPlayRTSPServer::AirPlayRTSPClientConnection::handleHTTPCmd_TunnelingGET(char const* sessionCookie) {
...
Boolean AirPlayRTSPServer::AirPlayRTSPClientConnection
::handleHTTPCmd_TunnelingPOST(char const* sessionCookie, unsigned char const* extraData, unsigned extraDataSize) {
  // Use the "sessionCookie" string to look up the separate "AirPlayRTSPClientConnection" object that should have been used to handle
  // an earlier HTTP "GET" request:
  if (fOurRTSPServer.fClientConnectionsForHTTPTunneling == NULL) {
    fOurRTSPServer.fClientConnectionsForHTTPTunneling = HashTable::create(STRING_HASH_KEYS);
  }
  AirPlayRTSPServer::AirPlayRTSPClientConnection* prevClientConnection
    = (AirPlayRTSPServer::AirPlayRTSPClientConnection*)(fOurRTSPServer.fClientConnectionsForHTTPTunneling->Lookup(sessionCookie));
  ...
  // Change the previous "AirPlayRTSPClientSession" object's input socket to ours.  It will be used for subsequent requests:
...
void AirPlayRTSPServer::AirPlayRTSPClientConnection::handleHTTPCmd_StreamingGET(char const* /*urlSuffix*/, char const* /*fullRequestStr*/) {
  // By default, we don't support requests to access streams via HTTP:
  handleHTTPCmd_notSupported();
}

void AirPlayRTSPServer::AirPlayRTSPClientConnection::resetRequestBuffer() {
...
void AirPlayRTSPServer::AirPlayRTSPClientConnection::closeSocketsRTSP() {
...
void AirPlayRTSPServer::AirPlayRTSPClientConnection::handleAlternativeRequestByte(void* instance, u_int8_t requestByte) {
  AirPlayRTSPClientConnection* connection = (AirPlayRTSPClientConnection*)instance;
  connection->handleAlternativeRequestByte1(requestByte);
}

void AirPlayRTSPServer::AirPlayRTSPClientConnection::handleAlternativeRequestByte1(u_int8_t requestByte) {
  if (requestByte == 0xFF) {
...
void AirPlayRTSPServer::AirPlayRTSPClientConnection::handleRequestBytes(int newBytesRead) {
  int numBytesRemaining = 0;
  ++fRecursionCount;
  
  do {
    AirPlayRTSPServer::AirPlayRTSPClientSession* clientSession = NULL;

    if (newBytesRead < 0 || (unsigned)newBytesRead >= fRequestBufferBytesLeft) {
      // Either the client socket has died, or the request was too big for us.
      // Terminate this connection:
#ifdef DEBUG
      fprintf(stderr, "AirPlayRTSPClientConnection[%p]::handleRequestBytes() read %d new bytes (of %d); terminating connection!\n", this, newBytesRead, fRequestBufferBytesLeft);
#endif
      fIsActive = False;
      break;
    }
...
#ifdef DEBUG
    ptr[newBytesRead] = '\0';
    fprintf(stderr, "AirPlayRTSPClientConnection[%p]::handleRequestBytes() %s %d new bytes:%s\n",
	...
	clientSession
	  = (AirPlayRTSPServer::AirPlayRTSPClientSession*)(fOurRTSPServer.lookupClientSession(sessionIdStr));
	if (clientSession != NULL) clientSession->noteLiveness();
	  ...
	  else if (strcmp(cmdName, "ANNOUNCE") == 0)
	  {
		  handleCmd_ANNOUNCE((char const*)fRequestBuffer);
	  }
	  ...
	  {/*
		  ...
		  if (!requestIncludedSessionId) {
			  ...
			  // So create a new "AirPlayRTSPClientSession" object for this request.
			  ...
			  if (authenticationOK("SETUP", urlTotalSuffix, (char const*)fRequestBuffer)) {
				  clientSession
					  = (AirPlayRTSPServer::AirPlayRTSPClientSession*)fOurRTSPServer.createNewClientSessionWithId();
			  }
			  else {
		  ...
		  */
		  handleCmd_SETUP((char const*)fRequestBuffer);
	  }
	  else if (strcmp(cmdName, "RECORD") == 0)
	  {
		  handleCmd_RECORD((char const*)fRequestBuffer);
	  }
	  else if (strcmp(cmdName, "FLUSH") == 0)
	  {
		  handleCmd_FLUSH((char const*)fRequestBuffer);
	  }
...
void AirPlayRTSPServer::AirPlayRTSPClientConnection::continueHandlingREGISTER(ParamsForREGISTER* params) {
  params->fOurConnection->continueHandlingREGISTER1(params);
}

void AirPlayRTSPServer::AirPlayRTSPClientConnection::continueHandlingREGISTER1(ParamsForREGISTER* params) {
  // Reuse our socket if requested:
  int socketNumToBackEndServer = params->fReuseConnection ? fClientOutputSocket : -1;

  AirPlayRTSPServer* ourServer = &fOurRTSPServer; // copy the pointer now, in case we "delete this" below
  ...
  if (socketNumToBackEndServer >= 0) {
    ...
    // "AirPlayRTSPClientConnection" object.  We do this now, in case the "implementCmd_REGISTER()" call below would also end up
    // deleting this.
    fClientInputSocket = fClientOutputSocket = -1; // so the socket doesn't get closed when we get deleted
    delete this;
  }
...
////////// AirPlayRTSPServer::AirPlayRTSPClientSession implementation //////////

AirPlayRTSPServer::AirPlayRTSPClientSession
::AirPlayRTSPClientSession(AirPlayRTSPServer& ourServer, u_int32_t sessionId)
...
AirPlayRTSPServer::AirPlayRTSPClientSession::~AirPlayRTSPClientSession() {
  reclaimStreamStates();
}

void AirPlayRTSPServer::AirPlayRTSPClientSession::deleteStreamByTrack(unsigned trackNum) {
...
void AirPlayRTSPServer::AirPlayRTSPClientSession::reclaimStreamStates() {
  for (unsigned i = 0; i < fNumStreamStates; ++i) {
    if (fStreamStates[i].subsession != NULL) {
...
void AirPlayRTSPServer::AirPlayRTSPClientConnection::AudioThread()
{
...
void AirPlayRTSPServer::AirPlayRTSPClientConnection::handleCmd_RECORD(char const* fullRequestStr)
{
	snprintf((char*)fResponseBuffer, sizeof fResponseBuffer,
		"RTSP/1.0 200 OK\r\n"
		"Server: AirTunes/105.1\r\n"
...
void AirPlayRTSPServer::AirPlayRTSPClientConnection::handleCmd_FLUSH(char const* fullRequestStr)
{
	snprintf((char*)fResponseBuffer, sizeof fResponseBuffer,
		"RTSP/1.0 200 OK\r\n"
		"Server: AirTunes/105.1\r\n"
...
void AirPlayRTSPServer::AirPlayRTSPClientConnection::handleCmd_SETUP(char const* fullRequestStr)
{
	//###
	// nodetunes does:
	...
	_pAudioSocket = new CUdpSocket();
	_pControlSocket = new CUdpSocket();
	_pTimingSocket = new CUdpSocket();

	_pAudioSocket->Initialise();
	_pControlSocket->Initialise();
	_pTimingSocket->Initialise();


	if (_pAudioSocket->GetPort() < 0 || _pControlSocket->GetPort() < 0 || _pTimingSocket->GetPort() < 0)
	{
	...
	else
	{
		snprintf((char*)fResponseBuffer, sizeof fResponseBuffer,
			...
			"Server: AirTunes/105.1\r\n"
			"CSeq: %s\r\n"
			"Transport: RTP/AVP/UDP;unicast;mode=record;server_port=%d;control_port=%d;timing port=%d\r\n"
			"Session: 1\r\n"
			"Audio-Jack-Status: connected\r\n\r\n",
			fCurrentCSeq,
			_pAudioSocket->GetPort(), _pControlSocket->GetPort(), _pTimingSocket->GetPort()
			);

		_pAudioThread = new std::thread(&AirPlayRTSPServer::AirPlayRTSPClientConnection::AudioThread, this);
	}
	...
	Server: AirTunes/105.1
	CSeq: ??\r\n
	Transport: RTP/AVP/UDP;unicast;mode=record;server_port=<audio port>;control_port=<control port>;timing port=<timing port>\r\n
	Session: 1\r\n
	Audio-Jack-Status: connected\r\n\r\n
	*/
	return;
...
void AirPlayRTSPServer::AirPlayRTSPClientSession::handleCmd_SETUP(AirPlayRTSPServer::AirPlayRTSPClientConnection* ourClientConnection, char const* urlPreSuffix, char const* urlSuffix, char const* fullRequestStr)
{

	
	//////////////////// original RTSP serverCSeq
...
void AirPlayRTSPServer::AirPlayRTSPClientSession
::handleCmd_withinSession(AirPlayRTSPServer::AirPlayRTSPClientConnection* ourClientConnection,
			  char const* cmdName,
...
void AirPlayRTSPServer::AirPlayRTSPClientSession
::handleCmd_TEARDOWN(AirPlayRTSPServer::AirPlayRTSPClientConnection* ourClientConnection,
		     ServerMediaSubsession* subsession) {
  unsigned i;
...
void AirPlayRTSPServer::AirPlayRTSPClientSession
::handleCmd_PLAY(AirPlayRTSPServer::AirPlayRTSPClientConnection* ourClientConnection,
  ...
  for (i = 0; i < fNumStreamStates; ++i) {
	...
	|| subsession == fStreamStates[i].subsession) {
      ...
      fStreamStates[i].subsession->startStream(fOurSessionId,
					       ...
					       AirPlayRTSPServer::AirPlayRTSPClientConnection::handleAlternativeRequestByte, ourClientConnection);
...
void AirPlayRTSPServer::AirPlayRTSPClientSession
::handleCmd_PAUSE(AirPlayRTSPServer::AirPlayRTSPClientConnection* ourClientConnection,
		  ServerMediaSubsession* subsession) {
  for (unsigned i = 0; i < fNumStreamStates; ++i) {
    if (subsession == NULL /* means: aggregated operation */
	|| subsession == fStreamStates[i].subsession) {
...
void AirPlayRTSPServer::AirPlayRTSPClientSession
::handleCmd_GET_PARAMETER(AirPlayRTSPServer::AirPlayRTSPClientConnection* ourClientConnection,
			  ServerMediaSubsession* /*subsession*/, char const* /*fullRequestStr*/) {
  // By default, we implement "GET_PARAMETER" just as a 'keep alive', and send back a dummy response.
  // (If you want to handle "GET_PARAMETER" properly, you can do so by defining a subclass of "AirPlayRTSPServer"
  // and "AirPlayRTSPServer::AirPlayRTSPClientSession", and then reimplement this virtual function in your subclass.)
  setRTSPResponse(ourClientConnection, "200 OK", fOurSessionId, LIVEMEDIA_LIBRARY_VERSION_STRING);
}

void AirPlayRTSPServer::AirPlayRTSPClientSession
::handleCmd_SET_PARAMETER(AirPlayRTSPServer::AirPlayRTSPClientConnection* ourClientConnection,
			  ServerMediaSubsession* /*subsession*/, char const* /*fullRequestStr*/) {
  // By default, we implement "SET_PARAMETER" just as a 'keep alive', and send back an empty response.
  // (If you want to handle "SET_PARAMETER" properly, you can do so by defining a subclass of "AirPlayRTSPServer"
  // and "AirPlayRTSPServer::AirPlayRTSPClientSession", and then reimplement this virtual function in your subclass.)
  setRTSPResponse(ourClientConnection, "200 OK", fOurSessionId);
}

GenericMediaServer::ClientConnection*
AirPlayRTSPServer::createNewClientConnection(int clientSocket, struct sockaddr_in clientAddr) {
  return new AirPlayRTSPClientConnection(*this, clientSocket, clientAddr);
}

GenericMediaServer::ClientSession*
AirPlayRTSPServer::createNewClientSession(u_int32_t sessionId) {
  return new AirPlayRTSPClientSession(*this, sessionId);
}


///////// AirPlayRTSPServerWithREGISTERProxying implementation /////////

AirPlayRTSPServerWithREGISTERProxying* AirPlayRTSPServerWithREGISTERProxying
::createNew(UsageEnvironment& env, Port ourPort,
  ...
  return new AirPlayRTSPServerWithREGISTERProxying(env, ourSocket, ourPort, authDatabase, authDatabaseForREGISTER, reclamationSeconds,
					    streamRTPOverTCP, verbosityLevelForProxying);
}

AirPlayRTSPServerWithREGISTERProxying
::AirPlayRTSPServerWithREGISTERProxying(UsageEnvironment& env, int ourSocket, Port ourPort,
				 UserAuthenticationDatabase* authDatabase, UserAuthenticationDatabase* authDatabaseForREGISTER,
				 unsigned reclamationSeconds,
				 Boolean streamRTPOverTCP, int verbosityLevelForProxying)
  : AirPlayRTSPServer(env, ourSocket, ourPort, authDatabase, reclamationSeconds),
...
char const* AirPlayRTSPServerWithREGISTERProxying::allowedCommandNames() {
  if (fAllowedCommandNames == NULL) {
    char const* baseAllowedCommandNames = AirPlayRTSPServer::allowedCommandNames();
...
Boolean AirPlayRTSPServerWithREGISTERProxying::weImplementREGISTER(char const* proxyURLSuffix, char*& responseStr) {
...
void AirPlayRTSPServerWithREGISTERProxying::implementCmd_REGISTER(char const* url, char const* /*urlSuffix*/, int socketToRemoteServer,
							   Boolean deliverViaTCP, char const* proxyURLSuffix) {
  ...
  // To change this default behavior, you will need to subclass "AirPlayRTSPServerWithREGISTERProxying", and reimplement this function.
...
UserAuthenticationDatabase* AirPlayRTSPServerWithREGISTERProxying::getAuthenticationDatabaseForCommand(char const* cmdName) {
  if (strcmp(cmdName, "REGISTER") == 0) return fAuthDBForREGISTER;

  return AirPlayRTSPServer::getAuthenticationDatabaseForCommand(cmdName);
}
