...
#ifndef _AIRPLAY_RTSP_SERVER_HH
#define _AIRPLAY_RTSP_SERVER_HH
...
public:
  static AirPlayRTSPServer* createNew(UsageEnvironment& env, Port ourPort = 554,
      ...
      // If "reclamationSeconds" > 0, then the "AirPlayRTSPClientSession" state for
  ...
  typedef void (responseHandlerForREGISTER)(AirPlayRTSPServer* rtspServer, unsigned requestId, int resultCode, char* resultString);
...
public: // should be protected, but some old compilers complain otherwise
  ...
  class AirPlayRTSPClientSession; // forward
  class AirPlayRTSPClientConnection: public GenericMediaServer::ClientConnection {
  public:
    // A data structure that's used to implement the "REGISTER" command:
    class ParamsForREGISTER {
    public:
      ParamsForREGISTER(AirPlayRTSPClientConnection* ourConnection, char const* url, char const* urlSuffix,
			Boolean reuseConnection, Boolean deliverViaTCP, char const* proxyURLSuffix);
      virtual ~ParamsForREGISTER();
    private:
      friend class AirPlayRTSPClientConnection;
      AirPlayRTSPClientConnection* fOurConnection;
      char* fURL;
      char* fURLSuffix;
  ...
  protected:
    ...
    virtual ~AirPlayRTSPClientConnection();

    friend class AirPlayRTSPServer;
    friend class AirPlayRTSPClientSession;
  ...
  protected:
    ...
    AirPlayRTSPServer& fOurRTSPServer; // same as ::fOurServer
  ...
  class AirPlayRTSPClientSession: public GenericMediaServer::ClientSession {
  protected:
    AirPlayRTSPClientSession(AirPlayRTSPServer& ourServer, u_int32_t sessionId);
    virtual ~AirPlayRTSPClientSession();

    friend class AirPlayRTSPServer;
    friend class AirPlayRTSPClientConnection;
    // Make the handler functions for each command virtual, to allow subclasses to redefine them:
    virtual void handleCmd_SETUP(AirPlayRTSPClientConnection* ourClientConnection,
				 char const* urlPreSuffix, char const* urlSuffix, char const* fullRequestStr);
    virtual void handleCmd_withinSession(AirPlayRTSPClientConnection* ourClientConnection,
					 char const* cmdName,
					 char const* urlPreSuffix, char const* urlSuffix,
					 char const* fullRequestStr);
    virtual void handleCmd_TEARDOWN(AirPlayRTSPClientConnection* ourClientConnection,
				    ServerMediaSubsession* subsession);
    virtual void handleCmd_PLAY(AirPlayRTSPClientConnection* ourClientConnection,
				ServerMediaSubsession* subsession, char const* fullRequestStr);
    virtual void handleCmd_PAUSE(AirPlayRTSPClientConnection* ourClientConnection,
				 ServerMediaSubsession* subsession);
    virtual void handleCmd_GET_PARAMETER(AirPlayRTSPClientConnection* ourClientConnection,
					 ServerMediaSubsession* subsession, char const* fullRequestStr);
  ...
  protected:
    ...
    // Shortcuts for setting up a RTSP response (prior to sending it):
    void setRTSPResponse(AirPlayRTSPClientConnection* ourClientConnection, char const* responseStr) { ourClientConnection->setRTSPResponse(responseStr); }
    void setRTSPResponse(AirPlayRTSPClientConnection* ourClientConnection, char const* responseStr, u_int32_t sessionId) { ourClientConnection->setRTSPResponse(responseStr, sessionId); }
    void setRTSPResponse(AirPlayRTSPClientConnection* ourClientConnection, char const* responseStr, char const* contentStr) { ourClientConnection->setRTSPResponse(responseStr, contentStr); }
    void setRTSPResponse(AirPlayRTSPClientConnection* ourClientConnection, char const* responseStr, u_int32_t sessionId, char const* contentStr) { ourClientConnection->setRTSPResponse(responseStr, sessionId, contentStr); }
...
protected: // redefined virtual functions
  // If you subclass "AirPlayRTSPClientConnection", then you must also redefine this virtual function in order
  // to create new objects of your subclass:
  virtual ClientConnection* createNewClientConnection(int clientSocket, struct sockaddr_in clientAddr);

protected:
  // If you subclass "AirPlayRTSPClientSession", then you must also redefine this virtual function in order
  // to create new objects of your subclass:
...
private:
  ...
  void noteTCPStreamingOnSocket(int socketNum, AirPlayRTSPClientSession* clientSession, unsigned trackNum);
  void unnoteTCPStreamingOnSocket(int socketNum, AirPlayRTSPClientSession* clientSession, unsigned trackNum);
...
private:
  ...
  HashTable* fClientConnectionsForHTTPTunneling; // maps client-supplied 'session cookie' strings to "AirPlayRTSPClientConnection"s
...
////////// A subclass of "AirPlayRTSPServer" that implements the "REGISTER" command to set up proxying on the specified URL //////////

class AirPlayRTSPServerWithREGISTERProxying: public AirPlayRTSPServer {
public:
  static AirPlayRTSPServerWithREGISTERProxying* createNew(UsageEnvironment& env, Port ourPort = 554,
						   UserAuthenticationDatabase* authDatabase = NULL,
...
protected:
  ...
  // called only by createNew();
  virtual ~AirPlayRTSPServerWithREGISTERProxying();
